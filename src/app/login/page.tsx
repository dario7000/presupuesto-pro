  ...
  return (
    <div className="min-h-screen bg-brand-gradient flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-1/4 left-1/4 w-1 h-1 bg-amber-400/30 rounded-full" />
        <div className="absolute top-1/3 right-1/3 w-1 h-1 bg-amber-400/20 rounded-full" />
        <div className="absolute bottom-1/4 right-1/4 w-1.5 h-1.5 bg-amber-400/20 rounded-full" />
      </div>
      ...
      <div className="w-full max-w-[380px] relative animate-fade-in">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-extrabold text-white mb-1 tracking-tight" style={{ fontFamily: 'var(--font-heading)' }}>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-white hover:bg-gray-50 text-gray-700 font-semibold rounded-xl text-sm transition-all mb-5 shadow-sm hover:shadow-md"
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <span className="text-xs text-slate-500 font-medium uppercase tracking-wider">o con email</span>
          <div className="flex-1 h-px bg-white/10" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            placeholder="tu@email.com" required className="input-dark" />
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            placeholder="Contraseña (mínimo 6 caracteres)" required minLength={6} className="input-dark" />
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full mt-1">
            {loading ? 'Cargando...' : isRegister ? 'Crear cuenta gratis' : 'Ingresar'}
          </button>
        </form>
        ...
        <p className="text-center mt-6 text-sm text-slate-500">
          ...
          >
            {isRegister ? 'Ingresá' : 'Registrate gratis'}
          </button>
        </p>
