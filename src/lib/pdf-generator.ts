...
// Generate initials-based logo as base64 image
...
export async function generateQuotePDF(
  ...
  if (business.logoUrl) {
    logoImg = await loadImageAsBase64(business.logoUrl)
  }
  if (!logoImg) {
    logoImg = generateInitialsLogo(business.businessName || business.ownerName)
  }

  if (logoImg) {
    try {
      doc.addImage(logoImg, 'PNG', margin, 6, 26, 26)
    } catch (e) {
  ...
  const details = [business.trade, business.phone, business.city].filter(Boolean).join('  •  ')
  doc.text(details, margin + 32, 23)

  if (business.address) {
    doc.text(business.address, margin + 32, 29)
  }
  ...
  if (quote.validUntil) {
    const validStr = new Date(quote.validUntil).toLocaleDateString('es-AR', {
      day: '2-digit', month: 'long', year: 'numeric'
    })
    doc.text(`Válido hasta: ${validStr}`, pageWidth - margin - 6, y + 16, { align: 'right' })
  }
  ...
  doc.roundedRect(margin, y, contentWidth, quote.vehicleInfo ? 28 : 20, 3, 3, 'F')
  
  // Amber left border
  doc.setFillColor(...amber)
  doc.rect(margin, y, 3, quote.vehicleInfo ? 28 : 20, 'F')
  ...
  const tableStyles = {
    ...
    columnStyles: {
      ...
      3: { cellWidth: 32, halign: 'right' as const, fontStyle: 'bold' as const },
    },
  ...
  if (labor.length > 0) {
    ...
    autoTable(doc, {
      startY: y,
      head: [['Descripción', 'Cant.', 'Precio Unit.', 'Subtotal']],
      body: buildTableRows(labor),
      ...tableStyles,
    })
    y = (doc as any).lastAutoTable.finalY + 4
  }

  // If items without specific category
  const others = quote.items.filter(i => i.category !== 'material' && i.category !== 'mano_de_obra')
  if (others.length > 0) {
    autoTable(doc, {
      startY: y,
      head: [['Descripción', 'Cant.', 'Precio Unit.', 'Subtotal']],
      body: buildTableRows(others),
      ...tableStyles,
    })
    y = (doc as any).lastAutoTable.finalY + 4
  }
  ...
  // TOTALS BOX
  // ==========================================
  const totalsHeight = quote.discountPercent > 0 ? 32 : 22
  doc.setFillColor(...dark)
  doc.roundedRect(pageWidth - margin - 80, y, 80, totalsHeight, 3, 3, 'F')
  ...
  if (quote.discountPercent > 0) {
    ...
    doc.setFontSize(8)
    doc.setTextColor(148, 163, 184)
    doc.text('Subtotal:', pageWidth - margin - 74, ty)
    doc.text(formatARS(quote.subtotal), pageWidth - margin - 6, ty, { align: 'right' })
    ty += 6

    doc.setTextColor(239, 68, 68)
    doc.text(`Descuento (${quote.discountPercent}%):`, pageWidth - margin - 74, ty)
  ...
  doc.setFontSize(11)
  doc.setTextColor(...amber)
  doc.text('TOTAL:', pageWidth - margin - 74, ty)
  ...
  if (options.watermark) {
    ...
    const centerY = 150
    doc.text('MUESTRA', centerX, centerY, {
      align: 'center',
      angle: 45,
    })
